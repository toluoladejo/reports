...
$localReports | ForEach-object {
    ...
    if ($properties.Length -gt 0){
        ...
        for ($a = 0; $a -lt $total; $a++){
            ...
            $reportDatasources = Get-RsItemDataSource -ReportServerUri $SSRSWebServiceUrl -RsItem $reportPath"/"$filename | Out-String
            $dataSourceName = $reportDatasources.Name | Out-String
            if ($dataSourceName  -match $reportDatasourceName){
                # Datasource
