...
$localReports | ForEach-object {
    ...
    if ($properties.Length -gt 0){
        ...
        Write-Host "Total References: "$properties.Length
        Write-Host "Connecting in progress......"

        for ($a = 0; $a -lt $total; $a++){

            $reportDataReferenceName =  $reportDatasourceNames[$a]
            $reportDataReferencePath = $reportDatasourcePaths[$a]

            #Check if path is datasource
            $reportDatasources = Get-RsItemDataSource -ReportServerUri $SSRSWebServiceUrl -RsItem $reportPath"/"$filename
            $dataSourceName = $reportDatasources.Name 
            
            if ($dataSourceName  -match $reportDataReferenceName){
                # Datasource
                Set-RsDataSourceReference -ReportServerUri $SSRSWebServiceUrl -Path $report.path -DataSourceName $reportDataReferenceName -DataSourcePath $reportDataReferencePath -Verbose  
            
                if ($?) {
                    Write-Host "Successfully connected "$report.path" report to $reportDataReferencePath datasource" 
                }else{
                    write-Warning "Error occured while connecting "$report.path" report to $reportDataReferencePath datasource" 
                } 
                
            }else{
                # Dataset 
                Set-RsDataSetReference -ReportServerUri $SSRSWebServiceUrl -Path $report.path -DataSetName $reportDataReferenceName -DataSetPath  $reportDataReferencePath

                if ($?) {
                    Write-Host "Successfully connected "$report.path" report to $reportDataReferencePath dataset" 
                }else{
                    write-Warning "Error occured while connecting "$report.path" report to $reportDataReferencePath dataset" 
                }
            }    
        }
